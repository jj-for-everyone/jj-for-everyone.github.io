...
The only thing we can do is to recreate it:
...
As we quickly touched on in level 1, Jujutsu has a concept of bookmarks being **tracked** or **untracked**.
...
A bookmark created with `jj bookmark create` needs to be tracked manually, while a bookmark generated with `jj git push --change` is automatically tracked.

When you clone a repository, only the `main` bookmark will automatically become tracked.
All other bookmarks that exist on the remote remain untracked by default.
That's a good default, because you may be working together with many other people on a project.
