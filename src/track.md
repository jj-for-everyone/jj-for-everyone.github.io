...
It's just hidden by default, but we can find it with `jj log --revision 'all()'`:
...
We can fix this by _tracking_ the bookmark, but let's talk about what that means in detail.
